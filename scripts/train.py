...
def main(cfg: DictConfig) -> None:
    ...
    ) = build_target_layout(cfg, train_store)
    ...
    train_loader = get_dataloader(
        ...
        subset_seed=int(cfg.train.seed),
        **dataloader_cfg,
    )
    val_loader = get_dataloader(
        split="val",
        split_fraction=val_fraction,
        subset_seed=int(cfg.train.seed) + 1,
        **dataloader_cfg,
    )
    ...
    early_stopping_monitor = str(
        OmegaConf.select(cfg, "train.early_stopping.monitor") or "val_rmse_splot"
    ).lower()
    early_stopping_mode = str(
        OmegaConf.select(cfg, "train.early_stopping.mode") or "min"
    ).lower()
    if early_stopping_mode not in {"min", "max"}:
        raise ValueError(
            f"Unsupported early_stopping.mode='{early_stopping_mode}'. Use 'min' or 'max'."
        )
    ...
    if early_stopping_enabled:
        ...
        console.print(
            f"  Monitor:             [cyan]{early_stopping_monitor} ({early_stopping_mode})[/cyan]"
        )
    ...
    for epoch in range(cfg.train.epochs):
        ...
        monitored_value = float("nan")
        if early_stopping_monitor == "val_loss":
            monitored_value = float(val_loss_avg)
        elif early_stopping_monitor in {"val_rmse_splot", "val_rmse"}:
            if val_metric_summary is not None:
                monitored_value = float(val_metric_summary["rmse"])
        elif early_stopping_monitor in {"val_mae_splot", "val_mae"}:
            if val_metric_summary is not None:
                monitored_value = float(val_metric_summary["mae"])
        elif early_stopping_monitor in {"val_r2_splot", "val_r2"}:
            if val_metric_summary is not None:
                monitored_value = float(val_metric_summary["r2"])
        else:
            raise ValueError(
                "Unsupported train.early_stopping.monitor='"
                f"{early_stopping_monitor}'. Supported: "
                "val_loss, val_rmse_splot, val_mae_splot, val_r2_splot."
            )

        monitor_valid = math.isfinite(monitored_value)
        if not monitor_valid:
            console.print(
                "[yellow]Monitored validation metric is NaN — skipping checkpoint and early-stopping update.[/yellow]"
            )

        if early_stopping_mode == "min":
            is_best = monitor_valid and (
                monitored_value < best_monitor_value - early_stopping_min_delta
            )
        else:
            is_best = monitor_valid and (
                monitored_value > best_monitor_value + early_stopping_min_delta
        ...
        if is_best:
            best_monitor_value = monitored_value
            best_epoch = epoch + 1
            epochs_without_improvement = 0
            torch.save(
                {
            ...
            console.print(
                "[green]New best model saved[/green] "
                f"({early_stopping_monitor}={best_monitor_value:.6f})"
            )
        elif monitor_valid:
        ...
        if cfg.wandb.enabled:
            ...
            if val_metric_summary is not None:
                ...
                log_dict["val/mae"] = val_metric_summary["mae"]
            log_dict[f"val/{early_stopping_monitor}"] = monitored_value
            if is_best:
                log_dict[f"val/{early_stopping_monitor}_best"] = best_monitor_value
    ...
    best_metric_str = (
        f"{best_monitor_value:.6f}" if math.isfinite(best_monitor_value) else "nan"
    )
    console.print(
        f"Best {early_stopping_monitor}={best_metric_str} at epoch {best_epoch}"
    )
    ...
    if cfg.wandb.enabled:
        ...
        run.summary["monitor_name"] = early_stopping_monitor
        run.summary["monitor_mode"] = early_stopping_mode
