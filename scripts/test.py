...
def main(cfg: DictConfig) -> None:
    ...
    with torch.no_grad():
        for X, y, src in track(
            ...
            eval_source_value = OmegaConf.select(
                train_cfg, "data.targets.eval_source_value"
            )
            if eval_source_value is None:
                eval_source_value = 2 if source_indices else source_value_if_missing
            eval_source_mask = src == float(eval_source_value)
            y = torch.where(eval_source_mask, y, torch.nan)

            valid = torch.isfinite(y_pred) & torch.isfinite(y) & eval_source_mask
