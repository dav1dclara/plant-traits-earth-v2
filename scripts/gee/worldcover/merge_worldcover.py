...
Usage:
    python merge_worldcover.py
    python merge_worldcover.py --input-dir data/1km/predictors_new/worldcover --output worldcover_land_mask.tif
"""
...
def main():
    ...
    parser.add_argument(
        "--input-dir", type=Path, default=Path("data/1km/predictors_new/worldcover")
    )
    ...
    parts = sorted(
        p
        for p in args.input_dir.glob("worldcover_land_mask_*.tif")
    ...
    # All files on disk are guaranteed to have valid data (download script discards
    # all-nodata tiles). Compute output bbox from transforms only — no data read needed.
    ...
    output = args.output or args.input_dir / "worldcover_land_mask.tif"
    ...
    with rasterio.open(
        ...
        dst.write(np.full((1, out_height, out_width), NODATA, dtype=np.uint8))
    ...
    with progress, rasterio.open(output, "r+") as dst:
        ...
        for part in parts:
            ...
            existing[patch != NODATA] = patch[patch != NODATA]
