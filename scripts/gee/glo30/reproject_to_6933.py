"""
Reproject the GLO-30 DEM to the exact grid of a reference raster using average
resampling. Water masking is handled separately after reprojection.

The output will have the same CRS, transform, width, and height as the reference
file. NaN nodata is preserved.

Usage:
    python reproject_to_6933.py
    python reproject_to_6933.py \
        --input data/1km/predictors_new/glo30/copernicus_glo30.tif \
        --reference data/1km/targets/gbif/X4.tif \
        --output data/1km/predictors_new/glo30/copernicus_glo30_6933.tif
...
INPUT_DEFAULT = Path("data/1km/predictors_new/glo30/copernicus_glo30.tif")
