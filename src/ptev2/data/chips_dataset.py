"""Dataset and dataloader for chips_centered zarr (any patch size).
...
Supported patch sizes: 3×3, 5×5, 7×7 (auto-detected from zarr attrs).
Center pixel index: patch_size // 2  (1 for 3×3, 2 for 5×5, 3 for 7×7).

Zarr structure expected (chips_centered/patch{N}_stride1/{split}.zarr):
    predictors/{name}               (N, C, P, P)  float32   P = patch_size
    targets/supervision_splot_only  (N, 74, P, P) float32   interleaved: [val_t0, src_t0, ...]
    targets/supervision_gbif_only   (N, 74, P, P) float32   interleaved: [val_t0, src_t0, ...]
    center/gbif_mean                (N, 37)        float32
    center/gbif_valid               (N, 37)        uint8   (1 = valid GBIF at center)
    center/splot_mean               (N, 37)        float32
    center/splot_valid              (N, 37)        uint8   (1 = valid SPLOT at center)
...
def get_chips_dataloader(
    ...
    """Return a DataLoader, auto-detecting zarr format (chips_centered vs patch15).

    Detects format by checking for ``targets/supervision_splot_only`` (chips_centered)
    or ``targets/comb`` (patch15_stride10) and instantiates the appropriate Dataset.
    ...
    """
    _store = zarr.open_group(str(zarr_path), mode="r")
    ...
    if "supervision_splot_only" in _target_keys:
        ...
        raise ValueError(
            ...
            f"Expected 'supervision_splot_only' or 'comb' under targets/. "
